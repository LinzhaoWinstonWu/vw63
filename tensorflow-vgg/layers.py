...
def topK_error(predictions, labels, K=5):
    ...
    #
    #     tf.reshape(labels_i, (-1, 1))
    #     tf.reshape(predictions_i, (-1, 1))
    #
    #     correct = tf.cast(tf.nn.in_top_k(predictions_i, labels_i, K), tf.float32)
    #     accuracy = tf.reduce_mean(correct)
    #     error = error + (1.0 - accuracy)
    #
    # return error / labels.shape[1]
    return error
