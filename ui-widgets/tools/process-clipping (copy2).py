...
def createClippings(widgets_folder, meta_dump, further_remove):
    ...
    for w_type, widgets in widgets_folder.items():
        ...
        for widget in widgets:
            ...
            #     clip = im.crop((widget['coordinates']['from'][0],
            #                     widget['coordinates']['from'][1],
            #                     widget['coordinates']['from'][0]+widget['dimensions']['width'],
            #                     widget['coordinates']['from'][1]+widget['dimensions']['height']))
            ...
            print "clipping " + widget['src'] + ".png ---> " + w_type + " " + filename + ".png"
            ...
            if widget['leaf'] and widget['widget_class'] in target_list:
                ...
                if not widgets_folder[widget['widget_class']]:
                    ...
                    for w in widgets_folder[widget['widget_class']]:
                        if w['text'] == widget['text'] and \
                                w['content-desc'] == widget['content-desc'] and \
                                w['dimensions']['width'] == widget['dimensions']['width'] and \
                                w['dimensions']['height'] == widget['dimensions']['height']:
                            duplicate = True
                            break
                    if not duplicate:
                        widgets_folder[widget['widget_class']].append(widget)
...
if __name__ == '__main__':
    ...
    pool = Pool(processes=1, initializer=init, initargs=(count, hash_dict, stats,))
